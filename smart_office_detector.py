...
class SmartOfficeDetector:
    def __init__(self):
        """Initialize smart office detection system"""
        print("🏢 Initializing Smart Office Detection System...")
        
        # Load detection model
        self.model = YOLO('yolov8n.pt')
        
        # Initialize pose detection for posture monitoring
        ...
        self.alert_thresholds = {
            ...
            'poor_posture': 30,   # 30 seconds of poor posture
    ...
    def log_to_database(self, table: str, data: dict):
        ...
        try:
            ...
            elif table == "posture_alerts":
                cursor.execute('''
                    INSERT INTO posture_alerts (timestamp, person_id, posture_score, alert_type)
                    VALUES (?, ?, ?, ?)
                ''', (data['timestamp'], data['person_id'], data['score'], data['alert_type']))
    ...
    def analyze_posture(self, frame, person_detections):
        """Analyze posture for people in frame"""
        posture_results = []
        ...
        for detection in person_detections:
            if detection['class'] == 'person' and detection['confidence'] > 0.7:
                ...
                if results.pose_landmarks:
                    ...
                    posture_score = self.calculate_posture_score(results.pose_landmarks)
                    
                    posture_data = {
                        'person_id': detection.get('track_id', 0),
                        'posture_score': posture_score,
                        'status': 'good' if posture_score > 0.7 else 'poor',
                        'landmarks': results.pose_landmarks,
                        'bbox': box
                    }
                    
                    posture_results.append(posture_data)
                    
                    # Check for posture alerts
                    if posture_score < 0.5:  # Poor posture threshold
                        person_id = detection.get('track_id', 0)
                        self.check_posture_alert(person_id, posture_score)
    ...
    def calculate_posture_score(self, landmarks):
        ...
        try:
            ...
            # Combine metrics into score (0-1, higher is better)
            posture_score = 1.0 - (shoulder_diff + head_alignment + spine_alignment) / 3
    ...
    def check_posture_alert(self, person_id: int, posture_score: float):
        """Check if posture alert should be triggered"""
        current_time = time.time()
        
        if person_id not in self.posture_alerts:
            self.posture_alerts[person_id] = {
                'poor_posture_start': current_time,
                'last_alert': 0,
                'alert_count': 0
            }
        
        # Check if poor posture duration exceeds threshold
        poor_duration = current_time - self.posture_alerts[person_id]['poor_posture_start']
        
        if poor_duration > self.alert_thresholds['poor_posture']:
            # Only alert every 60 seconds to avoid spam
            if current_time - self.posture_alerts[person_id]['last_alert'] > 60:
                alert = {
                    'type': 'poor_posture',
                    'person_id': person_id,
                    'message': f"💺 Person {person_id} has poor posture for {poor_duration:.0f} seconds",
                    'severity': 'medium',
                    'timestamp': current_time,
                    'posture_score': posture_score
                }
                self.alert_queue.put(alert)
                self.posture_alerts[person_id]['last_alert'] = current_time
                self.posture_alerts[person_id]['alert_count'] += 1
                
                # Log to database
                self.log_to_database("posture_alerts", {
                    'timestamp': current_time,
                    'person_id': person_id,
                    'score': posture_score,
                    'alert_type': 'poor_posture_extended'
                })
    ...
    def draw_office_zones(self, frame):
        ...
        for zone in self.office_zones:
            ...
            cv2.rectangle(frame, (x1, y1 - bg_height), (x1 + bg_width, y1), (0, 0, 0), -1)
            cv2.addWeighted(frame[y1-bg_height:y1, x1:x1+bg_width], 0.7, 
                           frame[y1-bg_height:y1, x1:x1+bg_width], 0.3, 0, 
                           frame[y1-bg_height:y1, x1:x1+bg_width])
    ...
    def draw_office_dashboard(self, frame):
        ...
        # Alerts section
        y_offset += 40
        cv2.putText(frame, "Active Alerts:", (panel_x + 20, y_offset), 
    ...
    def process_office_frame(self, frame):
        ...
        # Analyze posture for people
        posture_results = self.analyze_posture(frame, people_detections)
        
        # Calculate processing time
        processing_time = time.time() - start_time
        fps = 1.0 / max(processing_time, 0.001)
        
        return detections, zone_counts, posture_results, fps
    ...
    def run_smart_office_monitoring(self):
        ...
        print("  P - Toggle posture monitoring")
        ...
        try:
            while True:
                ...
                detections, zone_counts, posture_results, fps = self.process_office_frame(frame)
                ...
                for detection in detections:
                    if detection['class'] == 'person':
                        ...
                        # Color based on posture if available
                        person_color = self.office_theme['occupied']
                        for posture in posture_results:
                            if np.array_equal(posture['bbox'], detection['box']):
                                if posture['status'] == 'good':
                                    person_color = self.office_theme['available']
                                else:
                                    person_color = self.office_theme['warning']
                                break
                ...
                if show_posture:
                    for posture in posture_results:
                        x1, y1, x2, y2 = map(int, posture['bbox'])
                        status_color = self.office_theme['available'] if posture['status'] == 'good' else self.office_theme['alert']
                        cv2.putText(frame, f"Posture: {posture['status']} ({posture['posture_score']:.2f})", 
                                   (x1, y2+20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, status_color, 1)
                ...
                if key == ord('q'):
                    ...
                    print(f"Posture monitoring: {'ON' if show_posture else 'OFF'}")
                elif key == ord('d'):
...
def main():
    ...
    print("🏢 Smart Office Detection System")
    print("=" * 60)
    
    try:
        detector = SmartOfficeDetector()
        detector.run_smart_office_monitoring()
    except Exception as e:
        print(f"❌ Error: {e}")
        print("💡 Make sure you have all dependencies installed:")
        print("   pip install ultralytics opencv-python mediapipe")
